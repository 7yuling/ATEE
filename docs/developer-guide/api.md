...
- `GET /v1/admin/async-reviews?status=pending`
- `POST /v1/admin/async-reviews/run`
...
- Async AI review queue list, manual processing, and background worker processing work through `/v1/admin/async-reviews`.
...
## Admin Async AI Review Queue

List async AI review jobs:

```text
GET /v1/admin/async-reviews?status=pending
```

`status` can be `pending`, `retry`, `processing`, `completed`, `dead_letter`, or `all`. List responses return queue summaries and completed decision summaries; they do not include raw request bodies or secret configuration values.
...
Processing claims due `pending` and `retry` jobs, calls the configured LLM gateway for AI review, applies the local decision engine and Tool Gateway, then stores a completed result summary. Provider failures become `retry`; after the max attempt count they become `dead_letter`.

The background worker is controlled by:

- `async_review_worker_enabled`: starts the worker with Core Service when true.
- `async_review_worker_interval_seconds`: polling interval for due jobs.
- `async_review_worker_batch_size`: maximum jobs processed per worker pass.

Keep the worker disabled during first-time production wiring if you want to inspect queued evidence manually before enabling automatic AI review spend.

For a local worker rehearsal without live provider traffic:

```bash
python scripts/async-ai-review-worker-smoke.py --report reports/async-ai-review-worker-smoke.md
```

The smoke covers background processing, budget exhaustion, and circuit-breaker behavior with local fake providers. Add `--include-live` only when a configured-provider call is intentional.
