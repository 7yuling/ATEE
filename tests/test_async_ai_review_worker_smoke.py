...
class AsyncAIReviewWorkerSmokeScriptTests(unittest.TestCase):
    def test_fake_worker_smoke_covers_budget_and_circuit_without_leaking_provider_details(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(scenarios["budget_exhaustion_dead_letter"]["provider_calls"], 1)
            self.assertEqual(scenarios["budget_exhaustion_dead_letter"]["completed"], 1)
            self.assertEqual(scenarios["budget_exhaustion_dead_letter"]["dead_letter"], 1)
